...
function parse_property_xml($property_xml) {
    ...
    if ($property.length > 0) {
        ...
        set_property_save_link($property);
    } else {
        $('#Property-Navigation div.view-controls').hide();
...
function set_property_directions_link($property) {
    $('#Property-Navigation div.view-controls').show();
...
function set_property_save_link($property) {
    if (has_local_storage) {
        $('#Property-Navigation span.save').show();
        $('#Property-Navigation span.save').attr('mls', $property.find("mls").text());
        $('#Property-Navigation span.save').unbind('click').click(function () {
            // retrieve saved
            var stored_properties = localStorage["stored_props"];
            if (stored_properties != null)
...
function load_property_images($property) {
    ...
    if ($images.length > 0) {
        ...
        for (var image = 0; image < $images.length; image++) {
            ...
            images_content += '<td><p style="background-image: url(' + $image.text() + ');"></p></td>';
        }
